...
class OrientationTestResult(Res):
    ...
    alignment_quality: float = Field(
        default=0.0, description="Jaccard overlap of the two mark trains at that offset. "
    ...
    alignment_source: str = Field(
        default="", description="⭐ What the clock alignment actually rested on: 'ttl' = the "
        "digital time-stamp the rig itself wrote (the file's top-level `bits` — sample-accurate, "
        "no decoder involved, preferred whenever present), 'lamp' = the 2P-lamp episodes found "
        "in the electrical trace (the distrusted signal of issue 003), 'none' = the clocks were "
        "not aligned at all. The caveat is computed from this and must be shown with the result.")
    caveat: str = Field(default="", description="Why this result cannot be taken at face value — "
                        "computed from `alignment_source`, one wording per grounds. The UI shows "
                        "it verbatim, never behind a `?`.")
...
class MeaShelfEntry(Res):
    ...
    missing: bool = Field(
        default=False,
        description="⛔ **There is no file to read at either address** — his original is not where "
        "he left it and no copy has landed. Said on the shelf as a live warning; the row refuses "
        "to open rather than showing empty numbers. Every field below is null in this state.",
    )
    source_present: bool = Field(
        default=True,
        description="⭐ **Is HIS OWN file at `source_path` right now?** Stat'ed on every read, "
        "never cached from import time, because a drive that was unplugged can come back. ⚠️ It "
        "exists for exactly one decision: `stored` **and not** `source_present` is the one case "
        "where removing a recording destroys the last copy of it, and the only case that asks him "
        "first (his ruling, 2026-08-14). ⛔ Do not read it as `missing` — a `referenced` recording "
        "with no source has no copy either, so removing THAT destroys nothing.",
    )
    duration_s: float | None = None
    n_channels: int | None = None
...
class MeaEnvelopeStatus(Res):
    ...
    started: list[str] = Field(
        ...
        "everything was already built. ⚠️ **Not strictly 'started by this call':** a `POST` issued "
        "while an earlier backfill is still running reports that earlier job's id rather than "
        "starting a duplicate, which is what a caller waiting on it actually wants.",
    )
